...
extern void set_smem_crash_system_pronto(void);
extern void set_smem_crash_system_adsp(void);
...
static irqreturn_t smsm_irq_handler(int irq, void *data)
{
	...
	if (irq == INT_ADSP_A11_SMSM) {
		...
		unsigned adsp = __raw_readl(SMSM_STATE_ADDR(SMSM_Q6_STATE));

		if (adsp & SMSM_RESET) {
			set_smem_crash_system_adsp();
	...
	if (!smsm_info.state) {
		...
		unsigned pronto = __raw_readl(SMSM_STATE_ADDR(SMSM_WCNSS_STATE));
		unsigned adsp = __raw_readl(SMSM_STATE_ADDR(SMSM_Q6_STATE));
		...
		} else if (pronto & SMSM_RESET) {
			set_smem_crash_system_pronto();
			set_smem_crash_kind_fatal();
			{
				char buf[33];
				memset( buf, '\0', sizeof(buf) );
