...
typedef struct {
    ...
    unsigned int sbl1_bootver[ ( KCC_SMEM_SBL1_BOOTVER_SIZE + 4 ) / sizeof(unsigned int) ];
    ...
    unsigned int fastboot_flag[ ( KCC_SMEM_FASTBOOT_FLAG_SIZE + 4 ) / sizeof(unsigned int) ];
...
void *kc_smem_alloc(unsigned smem_type, unsigned buf_size)
{
	...
	switch (smem_type)
	{
        ...
        case SMEM_SBL1_BOOTVER:
            if( buf_size <= KCC_SMEM_SBL1_BOOTVER_SIZE )
            {
                return( ( void * )&kc_smem_addr->sbl1_bootver[ 0 ] );
            }
            break;
        ...
        case SMEM_FASTBOOT_FLAG:
            if( buf_size <= KCC_SMEM_FASTBOOT_FLAG_SIZE )
            {
                return( ( void * )&kc_smem_addr->fastboot_flag[ 0 ] );
            }
            break;
