...
module_param(restart_level, int, S_IRUGO);
...
enum {
	SSR_RAMDUMP_DISABLE = 0,
	SSR_RAMDUMP_ENABLE  = 1,
};
/**
...
extern void get_smem_crash_system(char* crash_system, int size);
...
static int __init subsys_restart_init(void)
{
	...
	restart_level = (opt_bit == 0) ? SSR_LEVEL_RELATED : SSR_LEVEL_SYSTEM;

	ramdump_opt_bit = oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_02, 0x03);
	enable_ramdumps = (ramdump_opt_bit == 0) ? SSR_RAMDUMP_DISABLE : SSR_RAMDUMP_ENABLE;
