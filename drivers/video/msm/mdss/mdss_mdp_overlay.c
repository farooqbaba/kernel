...
static void mdss_mdp_overlay_update_pm(struct mdss_overlay_private *mdp5_data)
{
	...
	if (mutex_lock_interruptible(&mdp5_data->ctl->lock))
		return;

	if (!mdp5_data->ctl->power_on) {
		pr_info("%s: power down. skip.\n", __func__);
...
int mdss_mdp_overlay_kickoff(struct msm_fb_data_type *mfd)
{
	...
	if (( mfd->panel_info->type == DTV_PANEL ) &&
	    ( ctl->play_cnt == 1 )){
		ret = mdss_mdp_limited_lut_igc_config(mdp5_data->ctl);
		if (ret)
			pr_err("%s Unable to config IGC LUT data", __func__);
	}
commit_fail:
