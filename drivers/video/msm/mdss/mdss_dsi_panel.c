...
void mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (!gpio_is_valid(ctrl_pdata->vmipi_18_en_gpio)) {
		pr_err("%s:%d, reset line not configured\n",
			   __func__, __LINE__);
	}

	MSM_FB_INFO("%s: enable = %d\n", __func__, enable);

	if (enable) {
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
		usleep_range(5000, 6000);
		gpio_set_value((ctrl_pdata->vmipi_18_en_gpio), 1);
		usleep_range(15000, 16000);
		gpio_set_value((ctrl_pdata->disp_en_gpio), 1);
		usleep_range(15000, 16000);
		if (regulator_enable((ctrl_pdata->shared_pdata).vdda_vreg)) {
			pr_err("%s: Failed to enable regulator.\n", __func__);
		}
		usleep_range(3000, 4000);

		/* Execut RST -> HI after initialize of mipi dsi */
		/* Move to mdss_dsi_panel_on() */
		...
		if (regulator_disable((ctrl_pdata->shared_pdata).vdda_vreg)) {
		...
		gpio_set_value((ctrl_pdata->vmipi_18_en_gpio), 0);
		usleep_range(5000, 6000);
	}
	MSM_FB_INFO("%s:-\n", __func__);
}
			       ...
			       struct mdss_panel_common_pdata *panel_data)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &panel_data->on_post_cmds,
		"qcom,panel-on-post-cmds", "qcom,on-post-cmds-dsi-state");
