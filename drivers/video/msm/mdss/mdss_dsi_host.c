		...
		struct dsi_cmd_desc *cmds, int cnt)
{
	...
	*/
	...
	for (i = 0; i < cnt; i++) {
		...
		dchdr = &cm->dchdr;
		if (dchdr->wait)
			usleep_range(dchdr->wait * 1000, (dchdr->wait + 1) * 1000);
		cm++;
	}
					...
					struct dsi_buf *tp)
{
	...
	MSM_FB_DEBUG("%s: TX command\n", __func__);
	for (i = 0; i < len; i+=4) {
		MSM_FB_DEBUG(" %02x: 0x%02x 0x%02x 0x%02x 0x%02x\n",
			i, bp[i], bp[i+1], bp[i+2], bp[i+3]);
	}
	...
	if (!wait_for_completion_timeout(&ctrl->dma_comp,
		...
		dsi_status = MIPI_INP((ctrl->ctrl_base) + 0x08);
		pr_err("  DSI_STATUS = %08x\n", dsi_status);
		pr_err("    INTERLEAVE_OP_CONTENTION %d\n", (dsi_status & (1<<31)) ? 1 : 0);
		pr_err("    PHY_RESET_BUSY           %d\n", (dsi_status & (1<<6)) ? 1 : 0);
		pr_err("    GENERIC_TRIGGER_BUSY     %d\n", (dsi_status & (1<<5)) ? 1 : 0);
		pr_err("    BTA_BUSY                 %d\n", (dsi_status & (1<<4)) ? 1 : 0);
		pr_err("    VIDEO_MODE_ENGINE_BUSY   %d\n", (dsi_status & (1<<3)) ? 1 : 0);
		pr_err("    CMD_MODE_MDP_BUSY        %d\n", (dsi_status & (1<<2)) ? 1 : 0);
		pr_err("    CMD_MODE_DMA_BUSY        %d\n", (dsi_status & (1<<1)) ? 1 : 0);
		pr_err("    CMD_MODE_ENGINE_BUSY     %d\n", (dsi_status & (1<<0)) ? 1 : 0);
	}
...
irqreturn_t mdss_dsi_isr(int irq, void *ptr)
{
	...
	MSM_FB_DEBUG("%s: isr=%x", __func__, isr);
