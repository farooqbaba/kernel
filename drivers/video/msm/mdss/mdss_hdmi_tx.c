...
static DEFINE_MUTEX(hpd_lock);
static DEFINE_MUTEX(tx_pm_lock);
...
static int hdmi_tx_init_features(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	if (hdmi_ctrl->present_hdcp) {
		...
		hdcp_init_data.hpd_state = &hdmi_ctrl->hpd_state;
	...
	bool down_mix)
{
	...
	if (NULL == pdev) {
		DEV_ERR("%s: invalid param of device.\n", __func__);
		return -EPERM;
	}
...
static void hdmi_tx_hpd_off(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	hdmi_hdcp_cancel_auth_work(hdmi_ctrl->feature_data[HDMI_TX_FEAT_HDCP]);
...
static int hdmi_tx_hpd_on(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	if (hdmi_ctrl->hpd_initialized) {
		...
		/* set timeout to 512ms for hardware debounce */
		reg_val = (DSS_REG_R(io, HDMI_HPD_CTRL) & ~0x1FFF) | 0x0200;
...
static int hdmi_tx_dev_init(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	hdmi_ctrl->workq = alloc_workqueue("hdmi_tx_workq", WQ_MEM_RECLAIM, 0);
	if (!hdmi_ctrl->workq) {
...
static int __devinit hdmi_tx_probe(struct platform_device *pdev)
{
	...
	kc_hdmi_tx_init(&(hdmi_ctrl->device_cdev));
...
static int __devexit hdmi_tx_remove(struct platform_device *pdev)
{
	...
	kc_hdmi_tx_exit(&(hdmi_ctrl->device_cdev));
...
int kc_hdmi_tx_open(struct inode *inode, struct file *file)
{
	struct hdmi_tx_ctrl *hdmi_ctrl =
		container_of(inode->i_cdev, struct hdmi_tx_ctrl, device_cdev);

	file->private_data = hdmi_ctrl;

	return 0;
}
EXPORT_SYMBOL_GPL(kc_hdmi_tx_open);

int kc_hdmi_tx_release(struct inode *inode, struct file *file)
{
	return 0;
}
EXPORT_SYMBOL_GPL(kc_hdmi_tx_release);

long kc_hdmi_get_hdcp(struct file *file, unsigned long *val)
{
	struct hdmi_tx_ctrl *hdmi_ctrl = (struct hdmi_tx_ctrl *)file->private_data;
	struct dss_io_data *io = NULL;
	...
	*val = DSS_REG_R_ND(io, QFPROM_RAW_FEAT_CONFIG_ROW0_LSB);
	DEV_DBG("%s: QFPROM_RAW_FEAT_CONFIG_ROW0_LSB = 0x%08lx\n", __func__, *val);

	return 0;
}
EXPORT_SYMBOL_GPL(kc_hdmi_get_hdcp);
