...
int kc_hdmi_ddc_read(struct hdmi_hdcp_ctrl *hdcp_ctrl,
	struct hdmi_tx_ddc_ctrl *ddc_ctrl, struct hdmi_tx_ddc_data *ddc_data)
{
	int rc = 0;
	u32 hpd_state;
	...
	hpd_state = *hdcp_ctrl->init_data.hpd_state;
	mutex_unlock(hdcp_ctrl->init_data.mutex);

	if (hpd_state) {
		rc = hdmi_ddc_read(ddc_ctrl, ddc_data);
	} else {
		DEV_ERR("%s: hpd_state false. Skip!\n", __func__);
	}

	return rc;
}

int kc_hdmi_ddc_write(struct hdmi_hdcp_ctrl *hdcp_ctrl,
	struct hdmi_tx_ddc_ctrl *ddc_ctrl, struct hdmi_tx_ddc_data *ddc_data)
{
	int rc = 0;
	u32 hpd_state;
...
static int hdmi_hdcp_authentication_part1(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	rc = kc_hdmi_ddc_read(hdcp_ctrl, hdcp_ctrl->init_data.ddc_ctrl, &ddc_data);
	if (rc) {
	...
	ddc_data.data_len = 8;
	ddc_data.what = "An";
	rc = kc_hdmi_ddc_write(hdcp_ctrl, hdcp_ctrl->init_data.ddc_ctrl, &ddc_data);
	if (rc) {
...
int hdmi_hdcp_isr(void *input)
{
	...
	if (HDCP_STATE_INACTIVE == hdcp_ctrl->hdcp_state) {
		DEV_ERR("%s: HDCP inactive. Just clear int and return. hdcp_int_val 0x%X\n",
			__func__, hdcp_int_val);
...
void hdmi_hdcp_cancel_auth_work(void *input)
{
	struct hdmi_hdcp_ctrl *hdcp_ctrl = (struct hdmi_hdcp_ctrl *)input;
	int rc = 0;
	 ...
	 * set the currect state to inactive.
	 */
	DEV_DBG("%s: %s: Cancel hdcp auth work\n", __func__, HDCP_STATE_NAME);
	rc = cancel_delayed_work_sync(&hdcp_ctrl->hdcp_auth_work);
	if (rc)
		DEV_ERR("%s: %s: Deleted hdcp auth work\n", __func__,
			HDCP_STATE_NAME);

} /* hdmi_hdcp_cancel_auth_work */
...
void hdmi_hdcp_deinit(void *input)
{
	...
	kfree(hdcp_ctrl);
	kc_hdmi_hdcp_exit(&(hdcp_ctrl->device_cdev));
} /* hdmi_hdcp_deinit */
...
void *hdmi_hdcp_init(struct hdmi_hdcp_init_data *init_data)
{
	...
	kc_hdmi_hdcp_init(&(hdcp_ctrl->device_cdev));

error:
	return (void *)hdcp_ctrl;
} /* hdmi_hdcp_init */

int kc_hdmi_hdcp_open(struct inode *inode, struct file *file)
{
	struct hdmi_hdcp_ctrl *hdcp_ctrl =
		container_of(inode->i_cdev, struct hdmi_hdcp_ctrl, device_cdev);

	file->private_data = hdcp_ctrl;

	return 0;
}
EXPORT_SYMBOL_GPL(kc_hdmi_hdcp_open);

int kc_hdmi_hdcp_release(struct inode *inode, struct file *file)
{
	return 0;
}
EXPORT_SYMBOL_GPL(kc_hdmi_hdcp_release);

long kc_hdmi_get_aksv(struct file *file, struct kc_hdmi_aksv *val)
{
	struct hdmi_hdcp_ctrl *hdcp_ctrl = (struct hdmi_hdcp_ctrl *)file->private_data;
	long ret = 0;
	struct dss_io_data *io;
	u8 aksv[5];
	...
	/* Fetch aksv from QFPROM, this info should be public. */
	val->lsb = DSS_REG_R(hdcp_ctrl->init_data.qfprom_io,
		HDCP_KSV_LSB);
	val->msb = DSS_REG_R(hdcp_ctrl->init_data.qfprom_io,
		HDCP_KSV_MSB);

	aksv[0] =  val->lsb        & 0xFF;
	aksv[1] = (val->lsb >> 8)  & 0xFF;
	aksv[2] = (val->lsb >> 16) & 0xFF;
	aksv[3] = (val->lsb >> 24) & 0xFF;
	aksv[4] =  val->msb        & 0xFF;

	/* check there are 20 ones in AKSV */
	if (hdmi_hdcp_count_one(aksv, 5) != 20) {
		DEV_ERR("%s: QFPROM AKSV chk failed (AKSV=%02lx%08lx)\n",
						__func__, val->msb, val->lsb);
		val->result = -1;
	}
	DEV_DBG("%s: AKSV=%02lx%08lx, result = %d\n", __func__, 
				val->msb, val->lsb, val->result);
error:
	return ret;
}
EXPORT_SYMBOL_GPL(kc_hdmi_get_aksv);
