			      ...
			      struct mdss_panel_common_pdata *panel_data)
{
	...
	ctrl_pdata->vmipi_18_en_gpio = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,vmipi_18_en-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->vmipi_18_en_gpio)) {
		pr_err("%s:%d, vmipi_18_en gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->vmipi_18_en_gpio, "vmipi_18_en");
		if (rc) {
			pr_err("request reset gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->vmipi_18_en_gpio);
			if (gpio_is_valid(ctrl_pdata->rst_gpio))
				gpio_free(ctrl_pdata->rst_gpio);
			if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
				gpio_free(ctrl_pdata->disp_en_gpio);
	...
	ctrl_pdata->on_post_cmds = panel_data->on_post_cmds;
