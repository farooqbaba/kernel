...
static int mdss_mdp_video_stop(struct mdss_mdp_ctl *ctl)
{
	...
	if (ctx->timegen_en) {
		MSM_FB_INFO("checkpoint: %s: execute OFF sequence START\n", __func__);
		...
		mdss_mdp_irq_disable(MDSS_MDP_IRQ_INTF_UNDER_RUN,
			ctl->intf_num);
		MSM_FB_INFO("checkpoint: %s: execute OFF sequence END\n", __func__);
	}
...
static int mdss_mdp_video_display(struct mdss_mdp_ctl *ctl, void *arg)
{
	...
	if (!ctx->timegen_en) {
		MSM_FB_INFO("checkpoint: %s: execute ON sequence START\n", __func__);
		...
		MSM_FB_INFO("checkpoint: %s: execute ON sequence END\n", __func__);
	}
