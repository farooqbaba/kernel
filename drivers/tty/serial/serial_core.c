...
static const char device_name_hsl9[64] = {0x74, 0x74, 0x79, 0x48, 0x53, 0x4C, 0x39};
...
static void uart_close(struct tty_struct *tty, struct file *filp)
{
	...
	if (strcmp(tty->name , device_name_hsl9) == 0)
	{
		if (uart_open_status == UART_OPEN_ST)
		{
			uart_current = get_current();
			if (hsl9_pid == uart_current->pid)
			{
	...
	printk( KERN_INFO "[uart_close]name:%s\n", tty->name);
