...
static void gp2ap_work_handler(struct work_struct *work)
{
	...
	if (data->ps_detection == 0)
	{
		...
		PSALS_LOG(KERN_NOTICE, "ABS_DISTANCE_REPORT(detect=%d)\n", 0);
	}
	else
	{
	...
	else
	{
		...
		else
		{
			...
			if (data->ps_detection != 0)
			{
				...
				input_report_abs(data->input_dev_ps, ABS_DISTANCE_REPORT, 0);
				input_sync(data->input_dev_ps);
				PSALS_LOG(KERN_NOTICE, "ABS_DISTANCE_REPORT(detect=%d) [PS sensor DISABLE]\n", 0);
			}
		}
	}
...
static void gp2ap_set_sensor_nv(unsigned long ulArg)
{
	...
	case en_NV_PROXIMITY_SENSOR_TEMP_I:
		PSALS_DBG_LOG(KERN_INFO, "en_NV_PROXIMITY_SENSOR_TEMP_I\n");
		...
		{
			PSALS_DBG_LOG(KERN_INFO, "nv_proximity_sensor_temp[%d]=0x%04X\n",
