...
static int override_phy_init = 0x00D0D5A4;
...
/* Enable Proprietary charger detection */
bool dwc_debug_log = false;
module_param(dwc_debug_log, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(dwc_debug_log, "Enable KC Log Output");
...
{
	return context ? context->ext_xceiv.bsv : false ;
}
...
static void dwc3_msm_dump_phy_info(struct dwc3_msm *mdwc)
{
	...
	/* schedule work for doing block reset for recovery from erratic error */
	queue_work(system_nrt_wq, &mdwc->usb_block_reset_work);

#if 0
	pr_err("%s: USB clks are %s, coreCLK rate=%d\n",
		__func__,
		(atomic_read(&mdwc->clks_on) ? "ON" : "OFF"),
		(u32)clk_get_rate(mdwc->core_clk));
	pr_err("%s: USB clks are %s, refCLK rate=%d\n",
		__func__,
		(atomic_read(&mdwc->clks_on) ? "ON" : "OFF"),
		(u32)clk_get_rate(mdwc->ref_clk));
	pr_err("%s: USB clks are %s, xoCLK rate=%d\n",
		__func__,
		(atomic_read(&mdwc->clks_on) ? "ON" : "OFF"),
		(u32)clk_get_rate(mdwc->xo_clk));
	pr_err("%s: USB clks are %s, ifaceCLK rate=%d\n",
		__func__,
		(atomic_read(&mdwc->clks_on) ? "ON" : "OFF"),
		(u32)clk_get_rate(mdwc->iface_clk));
	pr_err("%s: USB clks are %s, utmiCLK rate=%d\n",
		__func__,
		(atomic_read(&mdwc->clks_on) ? "ON" : "OFF"),
		(u32)clk_get_rate(mdwc->utmi_clk));
#endif
}
...
int msm_register_usb_ext_notification(struct usb_ext_notification *info)
{
	...
	usb_ext->notify(usb_ext->ctxt, 0, dwc3_ext_notify_online);
...
static void dwc3_block_reset_usb_work(struct work_struct *w)
{
	...
	pr_err("%s: vbus_active = %d, charger.chg_type = %d\n", __func__, mdwc->vbus_active, mdwc->charger.chg_type);
	...
	if(mdwc->vbus_active == 1)
	{
		   ...
		   mdwc->charger.chg_type == DWC3_SDP_CHARGER)
		{
			...
			dwc3_msm_qscratch_reg_init(mdwc, DWC3_CONTROLLER_RESET_EVENT);
			dwc3_msm_qscratch_reg_init(mdwc, DWC3_CONTROLLER_POST_RESET_EVENT);
		}
...
static bool dwc3_chg_det_check_linestate(struct dwc3_msm *mdwc)
{
	...
	kc_dbg(mdwc->dev,
		"= CHG DET I = : CHARGING_DET_OUTPUT_REG = 0x%08x in %s\n",
		chg_det, __func__);
	return chg_det & (3 << 8);
}
...
static void dwc3_chg_detect_work(struct work_struct *w)
{
	...
	kc_dbg(mdwc->dev, "= CHG DET I = : chg_state = %s\n",
		state_to_string(mdwc->chg_state));
	...
	case USB_CHG_STATE_WAIT_FOR_DCD:
		...
		kc_dbg(mdwc->dev, "= CHG DET I = : is_dcd = %d, tmout = %d\n",
			is_dcd, tmout);
		if (is_dcd || tmout) {
			if (is_dcd)
				dcd = true;
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		kc_dbg(mdwc->dev,
			"= CHG DET I = : vout = %d, dcd = %d, detect = %d\n",
			vout, dcd, prop_chg_detect);
		break;
	case USB_CHG_STATE_PRIMARY_DONE:
		...
		kc_dbg(mdwc->dev, "= CHG DET I = : vout = %d\n", vout);
		/* fall through */
	...
	case USB_CHG_STATE_DETECTED:
		...
		kc_dbg(mdwc->dev, "= CHG DET I = : chg_type = %s\n",
			chg_to_string(mdwc->charger.chg_type));
...
static void dwc3_start_chg_det(struct dwc3_charger *charger, bool start)
{
	...
	if (start == false) {
		...
		kc_dbg(mdwc->dev,
			"= CHG DET I = : canceling charging detection work\n");
	...
	charger->chg_type = DWC3_INVALID_CHARGER;
	kc_dbg(mdwc->dev,
		"= CHG DET I = : start charging detection work\n");
...
static int dwc3_msm_suspend(struct dwc3_msm *mdwc)
{
	...
	atomic_set(&mdwc->clks_on, 0);
	//atomic_set(&msm->clks_on, 0);
...
static int dwc3_msm_resume(struct dwc3_msm *mdwc)
{
	...
	atomic_set(&mdwc->clks_on, 1);
	//atomic_set(&msm->clks_on, 1);
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		if (mdwc->otg_xceiv && !mdwc->ext_inuse &&
			...
			kc_dbg(mdwc->dev,
				"= CHG DET I = : vbus detect %s\n", __func__);
			if ( mdwc->ext_xceiv.bsv && usb_mhl_cb_info)
