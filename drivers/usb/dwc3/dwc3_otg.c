...
module_param(vbus_detect_count, uint, S_IRUGO);
...
static int dwc3_otg_get_ext_vbus_property(enum power_supply_property psp)
{
	struct power_supply *ext_vbus_psy;
	union power_supply_propval ret = {0,};

	ext_vbus_psy = power_supply_get_by_name("ext-vbus");
	if (!ext_vbus_psy) {
		pr_err("%s: Unable to get ext_vbus power_supply\n", __func__);
		return 0;
	}
	ext_vbus_psy->get_property(ext_vbus_psy, psp, &ret);
	pr_debug("%s: Get ext_vbus property: psp = %d, val = %d\n", __func__, psp, ret.intval);
	return ret.intval;
}
...
static int dwc3_otg_set_power(struct usb_phy *phy, unsigned mA)
{
			...
			dotg->charger->chg_type == DWC3_PROPRIETARY_CHARGER)
	{
		...
		if (dwc3_otg_get_ext_vbus_property(POWER_SUPPLY_PROP_ONLINE)) {
			power_supply_type = POWER_SUPPLY_TYPE_MHL;
		}
	}
	else
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	kc_dbg(phy->dev, "= USB DET I = : otg_state = %s\n",
		kc_otg_state_to_string(phy->state));
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!test_bit(ID, &dotg->inputs)) {
			...
			kc_dbg(phy->dev, "= USB DET I = : No device in %s\n",
					__func__);
			pm_runtime_put_sync(phy->dev);
		}
		break;
	...
	case OTG_STATE_B_IDLE:
		if (!test_bit(ID, &dotg->inputs)) {
			...
			if (charger) {
				kc_dbg(phy->dev,
					"= USB DET I = : charger type %s\n",
					chg_to_string(charger->chg_type));
			...
			if(dwc3_otg_get_ext_vbus_property(POWER_SUPPLY_PROP_ONLINE)) {
				...
				dwc3_otg_set_power(phy, (dwc3_otg_get_ext_vbus_property(POWER_SUPPLY_PROP_CURRENT_MAX)/1000));
				dev_dbg(phy->dev, "MHL charger\n");
				work = 0;
				pm_runtime_put_sync(phy->dev);
			}
			else {
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(B_SESS_VLD, &dotg->inputs) ||
			...
			switch_set_state(&sdev, 0x00);
			kc_dbg(phy->dev,
				"= USB DET I = : vbus not detect in %s\n",
				__func__);
		}
		break;
