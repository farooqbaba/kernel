...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	chip->last_soc = oem_bms_correct_soc(chip->last_soc);
	soc = chip->last_soc;
...
static void backup_charge_cycle(struct qpnp_bms_chip *chip)
{
	...
	oem_param_cycles_backup(chip->charge_cycles,
							chip->charge_increase,
							oem_bms_get_deteriorationstatus(&batt_deterioration_status, chip->charge_cycles)
							);
}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_BMS_BATT_STATUS:
		val->intval = oem_bms_get_deteriorationstatus(&batt_deterioration_status, chip->charge_cycles);
		break;
	default:
		return -EINVAL;
	}
...
static void oem_bms_set_battery_data(struct qpnp_bms_chip *chip)
{
//	chip->rbatt_initial_lut = bms_oem_data.rbatt_initial_lut;
//	chip->soc_adjust_lut    = bms_oem_data.soc_adjust_lut;
//	chip->cycle_adjust_lut  = bms_oem_data.cycle_adjust_lut;
	chip->charge_cycles			= oem_param_cycles.last_chargecycles;
	chip->charge_increase		= oem_param_cycles.last_charge_increase;
...
#define VSENSE_THR_MEAS_OCV			0x14
...
static int __devinit qpnp_bms_probe(struct spmi_device *spmi)
{
	...
	{
		u8 temp;
		temp = VSENSE_THR_MEAS_OCV;
		rc = qpnp_write_wrapper(chip, (u8 *)&temp,
				chip->base + BMS1_S3_VSENSE_THR_CTL, 1);
		if (rc) {
			pr_err("Unable to set BMS1_S3_VSENSE_THR_CTL %d\n", rc);
		}
	}
