...
#ifdef CONFIG_OEM_CHARGER_STAND
#include <oem-charger_stand.h>
#endif
#ifdef CONFIG_OEM_CHARGER_PARM
#include <oem-charger_parm.h>
#endif
#ifdef CONFIG_OEM_HKADC
#include <oem-hkadc.h>
#endif
#ifdef CONFIG_OEM_CHARGER
...
#define CHG_PTH_CTL				0x40
#define DC_CHG_PTH_STS			0x0A
...
#define USB_OVP_THR				0x00
#define USB_OVP_THR_MASK		0xF0
#define DC_OVP_THR				0x00
#define DC_OVP_THR_MASK			0xF0
#define DC_VALID_BIT			BIT(7)
...
struct qpnp_chg_chip {
	...
	bool					oem_chg_vbatdet_change_flg;
};
...
{
	if (oem_qpnp_chg_set_appropriate_vbatdet(chip)) {
		return;
	}
...
#define OEM_USB_WAKELOCK_MSEC        2000
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			chip->oem_chg_vbatdet_change_flg = true;
			qpnp_chg_set_appropriate_vbatdet(chip);
			wake_unlock(&oem_charge_wake_lock);
			wake_lock_timeout(&oem_charge_wake_lock, msecs_to_jiffies(OEM_USB_WAKELOCK_MSEC));
			if (!qpnp_chg_is_dc_chg_plugged_in(chip))
				chip->chg_done = false;
		} else {
			wake_lock(&oem_charge_wake_lock);
			oem_chg_set_temporary_chg_param(chip);
			oem_chg_failed_clear(chip);
			schedule_delayed_work(&chip->eoc_work,
				msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
	...
	oem_chg_vbatt_ov_check_charger();
...
qpnp_chg_bat_if_batt_temp_irq_handler(int irq, void *_chip)
{
	...
	if (batt_temp_good == true) {
		schedule_delayed_work(&chip->eoc_work, msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
		chip->resuming_charging = false;
		chip->oem_chg_vbatdet_change_flg = false;
		qpnp_chg_set_appropriate_vbatdet(chip);
	}
	return IRQ_HANDLED;
}
...
#define OEM_DC_WAKELOCK_MSEC        2000
static irqreturn_t
qpnp_chg_dc_dcin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->dc_present ^ dc_present) {
		...
		if (!dc_present && !qpnp_chg_is_usb_chg_plugged_in(chip)) {
			...
			chip->oem_chg_vbatdet_change_flg = true;
			qpnp_chg_set_appropriate_vbatdet(chip);
			wake_lock_timeout(&oem_charge_wake_lock, msecs_to_jiffies(OEM_DC_WAKELOCK_MSEC));
		} else {
			oem_chg_failed_clear(chip);
			schedule_delayed_work(&chip->eoc_work,
				msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
...
qpnp_chg_chgr_chg_fastchg_irq_handler(int irq, void *_chip)
{
	...
	if ( (chip->resuming_charging) || (chip->oem_chg_vbatdet_change_flg) ){
		chip->resuming_charging = false;
		chip->oem_chg_vbatdet_change_flg = false;
		qpnp_chg_set_appropriate_vbatdet(chip);
	}
...
static enum power_supply_property pm_power_props_ac[] = {
	POWER_SUPPLY_PROP_ONLINE,
};
...
get_prop_batt_health(struct qpnp_chg_chip *chip)
{
	...
	if (oem_chg_is_dc_ov_check(chip) ) {
		return POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
	}
	
	if (oem_is_chg_vbatt_ov()) {
...
{
    if (false == oem_is_uim_dete()) {
		qpnp_chg_vddmax_set(chip, oem_param_charger.uim_undete_chg);
		return;
	}
...
#define MAX_CLOSEST_DELTA_MV			10
static void
qpnp_chg_adjust_vddmax(struct qpnp_chg_chip *chip, int vbat_mv)
{
	int delta_mv, closest_delta_mv, sign;

	if(oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 2)) {
		pr_debug("Exiting. Is in factory mode \n");
		return;
	}
	
    if (false == oem_is_uim_dete()) {
		pr_debug("Exiting no SIM \n");
		return;
	}
	...
	if(delta_mv < 0 && delta_mv > -MIN_DELTA_MV_TO_INCREASE_VDD_MAX) {
		pr_debug("vbat is not high enough to decrease vdd\n");
		return;
	}
	...
	closest_delta_mv = clamp(closest_delta_mv,
			-MAX_CLOSEST_DELTA_MV, MAX_CLOSEST_DELTA_MV);
	pr_debug("clamp closest delta mv = %d \n", closest_delta_mv);
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_CHGR_SUBTYPE:
		...
		chip->oem_chg_vbatdet_change_flg = true;
		rc = qpnp_chg_vbatdet_set(chip, chip->max_voltage_mv + chip->resume_delta_mv);
		if (rc) {
	...
	case SMBCL_USB_CHGPTH_SUBTYPE:
		...
		rc = qpnp_chg_masked_write(chip,
			chip->usb_chgpth_base + CHG_PTH_CTL,
			0x80,
			0x80, 1);
...
#ifdef CONFIG_OEM_CHARGER_CMD
extern void oem_chg_cmd_register(void *chip);
#endif
...
bool oem_qpnp_chg_set_appropriate_vbatdet(void *_chip)
{
	...
	if (false == oem_is_uim_dete()) {
		qpnp_chg_vbatdet_set(chip, oem_param_charger.uim_undete_chg - chip->resume_delta_mv);
		return true;
	}
...
int oem_chg_ibatmax_set(void *_chip, int chg_current)
{
	struct qpnp_chg_chip *chip = _chip;

	return qpnp_chg_ibatmax_set(chip, chg_current);
}
EXPORT_SYMBOL(oem_chg_ibatmax_set);

void oem_chg_set_appropriate_battery_current(void *_chip)
{
	struct qpnp_chg_chip *chip = _chip;

	qpnp_chg_set_appropriate_battery_current(chip);
}
EXPORT_SYMBOL(oem_chg_set_appropriate_battery_current);

int oem_chg_batt_fet_ctrl(void *_chip, int enable)
{
	struct qpnp_chg_chip *chip = _chip;

	return qpnp_chg_charge_en(chip, enable);
}
...
{
	struct qpnp_chg_chip *chip = _chip;
	pr_debug("buck_ctrl disable = %d\n", disable);
	return qpnp_chg_force_run_on_batt(chip, disable);
}
EXPORT_SYMBOL(oem_chg_buck_ctrl);

#ifdef CONFIG_OEM_CHARGER_PARM
void oem_chg_set_charge_param(struct qpnp_chg_chip *chip)
{
	chip->max_voltage_mv = oem_param_charger.normal_chg;
	chip->cool_bat_mv    = oem_param_charger.cool_chg;
	chip->warm_bat_mv    = oem_param_charger.warm_chg;
	
	if(oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 5)) {
		if (oem_param_charger.chg_cool_tmp != INT_MIN)
			chip->cool_bat_decidegc = oem_param_charger.chg_cool_tmp * 10;
		else
			chip->cool_bat_decidegc = INT_MIN;

		if (oem_param_charger.chg_warm_tmp != INT_MIN)
			chip->warm_bat_decidegc = oem_param_charger.chg_warm_tmp * 10;
		else
			chip->warm_bat_decidegc = INT_MIN;

		chip->resume_delta_mv     = oem_param_charger.initial_delta_volt;
		chip->min_voltage_mv      = oem_param_charger.maint_chg_vin;
		chip->max_bat_chg_current = oem_param_charger.i_chg_norm;
		chip->cool_bat_chg_ma     = oem_param_charger.i_chg_cool;
		chip->warm_bat_chg_ma     = oem_param_charger.i_chg_warm;
		chip->term_current        = oem_param_charger.i_chg_finish;
	}
	pr_debug("max_voltage_mv = %d cool_bat_mv = %d warm_bat_mv = %d \n",
			  chip->max_voltage_mv, chip->cool_bat_mv, chip->warm_bat_mv);
	pr_debug("cool_bat_decidegc = %d warm_bat_decidegc = %d \n",
			  chip->cool_bat_decidegc, chip->warm_bat_decidegc);
	pr_debug("resume_delta_mv = %d min_voltage_mv = %d \n",
			  chip->resume_delta_mv, chip->min_voltage_mv);
	pr_debug("max_bat_chg_current = %d cool_bat_chg_ma = %d warm_bat_chg_ma = %d term_current = %d \n",
			  chip->max_bat_chg_current, chip->cool_bat_chg_ma, chip->warm_bat_chg_ma, chip->term_current);
}
#endif

static int
oem_power_get_property_ac(struct power_supply *psy,
				  enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct qpnp_chg_chip *chip = container_of(psy, struct qpnp_chg_chip,
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		dc_online = qpnp_chg_is_dc_chg_plugged_in(chip);

		chip->usb_psy->get_property(chip->usb_psy,
		  POWER_SUPPLY_PROP_ONLINE, &ret);
		usb_online = ret.intval;
		
		chip->usb_psy->get_property(chip->usb_psy,
		  POWER_SUPPLY_PROP_TYPE, &ret);
		usb_type = ret.intval;
		
		if(usb_online && usb_type == POWER_SUPPLY_TYPE_USB_DCP)
		{
			usb_ac_online = 1;
		}
		
		val->intval = dc_online | usb_ac_online | oem_chg_get_boost_chg_state();
		pr_debug("dc_online=%d, usb_online=%d, usb_type=%d, usb_ac_online=%d\n",dc_online, usb_online, usb_type, usb_ac_online);
		break;
	default:
		pr_debug("power_supply_property is Invalid\n");
		return -EINVAL;
	}
	return 0;
}
...
int oem_chg_is_dc_ov_check(void *_chip)
{
	...
	rc = qpnp_chg_read(chip, &dc_valid_sts, chip->dc_chgpth_base + DC_CHG_PTH_STS, 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n", chip->dc_chgpth_base + DC_CHG_PTH_STS, rc);
		return rc;
	}
	pr_debug("DC_CHG_PTH_STS 0x%x\n", dc_valid_sts);
	
	
	if (!(dc_valid_sts & DC_VALID_BIT) && (dc_valid_sts & DC_OVP_BIT)) {
		return true;
	}
	return false;
}
EXPORT_SYMBOL(oem_chg_is_dc_ov_check);

int oem_chg_is_dc_chg_check(void *_chip)
{
	u8 dc_valid_sts;
	int rc;
	struct qpnp_chg_chip *chip = _chip;
	...
	rc = qpnp_chg_read(chip, &dc_valid_sts,
				 chip->dc_chgpth_base + DC_CHG_PTH_STS , 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n",
				chip->dc_chgpth_base + DC_CHG_PTH_STS, rc);
		return rc;
	...
	return (dc_valid_sts & DC_VALID_BIT) ? 1 : 0;
}
EXPORT_SYMBOL(oem_chg_is_dc_chg_check);

static int oem_chg_flag = 0;
void oem_chg_set_temporary_chg_param(void *_chip)
{
	struct qpnp_chg_chip *chip = _chip;

	qpnp_chg_ibatmax_set(chip, 500);
	qpnp_chg_vinmin_set(chip, 4800);
	oem_chg_flag = 1;
}
...
EXPORT_SYMBOL(oem_chg_set_appropriate_chg_param);

#define OEM_CHGR_CHG_FAILED_BIT	BIT(7)
#define OEM_CHGR_ATC_FAILED_BIT	BIT(7)
void oem_chg_failed_clear(void *_chip)
{
	struct qpnp_chg_chip *chip = _chip;
	int rc;

	pr_debug("chg_failed_clear triggered\n");

	rc = qpnp_chg_masked_write(chip,
		chip->chgr_base + CHGR_CHG_FAILED,
		OEM_CHGR_CHG_FAILED_BIT,
		OEM_CHGR_CHG_FAILED_BIT, 1);
	if (rc)
		pr_err("Failed to write chg_fail clear bit!\n");

	rc = qpnp_chg_masked_write(chip,
		chip->chgr_base + CHGR_ATC_FAILED,
		OEM_CHGR_ATC_FAILED_BIT,
		OEM_CHGR_ATC_FAILED_BIT, 1);
	if (rc)
		pr_err("Failed to write atc_fail clear bit!\n");
}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	oem_chg_vbatt_ov_work_init();
	oem_chg_stand_timer_work_init();
	...
	if (&chip->ac_psy) {
		chip->ac_psy.name = "ac";
		chip->ac_psy.type = POWER_SUPPLY_TYPE_AC;
		chip->ac_psy.properties = pm_power_props_ac;
		chip->ac_psy.num_properties = ARRAY_SIZE(pm_power_props_ac);
		chip->ac_psy.get_property = oem_power_get_property_ac;
...
#ifdef CONFIG_OEM_CHARGER
	oem_chargermonit_init(chip);
#endif

#ifdef CONFIG_OEM_CHARGER_STAND
	oem_chg_stand_init(chip);
#endif

#ifdef CONFIG_OEM_CHARGER_CMD
	oem_chg_cmd_register(chip);
#endif
