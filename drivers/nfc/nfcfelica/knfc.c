...
/* NFC_VFC_RST device */
#define MFD_NFC_VFC_RST_GPIO_NUM		45
...
#define CXD2235_POWER_GPIO_NO			MFD_PON_GPIO_NUM
...
static int __devinit pm_nfc_probe(struct platform_device *pdev);
static int __devexit pm_nfc_remove(struct platform_device *pdev);
...
static ssize_t nfc_vfc_rst_write(struct file *file, const char __user *data, size_t len, loff_t *ppos)
{
	...
	gpio_set_value(MFD_NFC_VFC_RST_GPIO_NUM , seton );
...
static int nfc_vfc_rst_release(struct inode *inode, struct file *file)
{
	...
	gpio_set_value(MFD_NFC_VFC_RST_GPIO_NUM , NFC_VFC_RST_DEV_LOW);
...
static int nfc_vfc_rst_init(void)
{
	...
	gpio_set_value(MFD_NFC_VFC_RST_GPIO_NUM , NFC_VFC_RST_DEV_HIGH );
...
static void rfs_poll_work_func(struct work_struct *work)
{
	...
	read_value = gpio_get_value_cansleep(g_nfc_rfs_gpio);
...
static int __devinit cen_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	struct cen_data *cen;
	int alloc_ret = 0;
...
static int __devinit pm_nfc_probe(struct platform_device *pdev)
{
	...
	rc = gpio_request(g_nfc_int_gpio, "nfc_rfs-gpio");
	if (rc) {
		pr_err("%s: gpio_request failed.\n", __func__);
		return -EINVAL;
	}
