	...
	uint16_t gpio_array_size)
{
...
#endif
	if (of_property_read_bool(of_node, "qcom,gpio-visp") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-visp", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-visp failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-visp invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VISP] =
			gpio_array[val];
		CDBG("%s qcom,gpio-visp %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VISP]);
	}
    ...
    if (of_property_read_bool(of_node, "qcom,gpio-vdig") == true) {
		...
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDD] =
			gpio_array[val];
		CDBG("%s qcom,gpio-vdig %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDD]);
	}
