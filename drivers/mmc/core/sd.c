...
static void mmc_sd_detect(struct mmc_host *host)
{
...
#ifdef CONFIG_MMC_PARANOID_SD_INIT
	while(retries) {
		printk("SD card:mmc_sd_detect(mmc_send_status)\n");
	...
	if (!retries) {
		...
		printk("SD card:mmc_sd_detect(_mmc_detect_card_removed)\n");
...
int mmc_attach_sd(struct mmc_host *host)
{
	...
	retries = 5;
	while (retries) {
		printk("SD card:mmc_attach_sd(mmc_sd_init_card)\n");
