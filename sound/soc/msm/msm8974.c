...
#define MSM8974_RCV_ON 1
#define MSM8974_RCV_OFF 0
#define MSM8974_RCVAMP_ENABLE    0x1
static int piezo_rcvamp_gpio;
static int msm8974_rcv_pamp;
static int msm8974_rcv_control;

oem_board_type board_type;
...
static void msm8974_ext_rcv_control(struct snd_soc_codec *codec)
{
	struct snd_soc_dapm_context *dapm = &codec->dapm;

	mutex_lock(&dapm->codec->mutex);

	if (msm8974_rcv_control == MSM8974_RCV_ON) {
...
static void msm8974_enable_rcv_amp_gpio(void)
{
    ...
    gpio_set_value_cansleep(piezo_rcvamp_gpio, 1);
	if(OEM_BOARD6_TYPE <= board_type){
...
static void msm8974_disable_rcv_amp_gpio(void)
{
    unsigned char wBuff[16];
    struct i2c_adapter *adapter;
    int res;
	...
	if(OEM_BOARD6_TYPE <= board_type){
        ...
        if (!adapter) {
            pr_err("%s: invalid i2c adapter\n", __func__);
    	    gpio_set_value_cansleep(piezo_rcvamp_gpio, 0);
...
static void msm8974_rcv_power_amp_on(u32 rcv)
{
    if (rcv & MSM8974_RCVAMP_ENABLE) {
        if (msm8974_rcv_pamp & MSM8974_RCVAMP_ENABLE) {
            return;
        }
        msm8974_rcv_pamp |= rcv;
        msm8974_enable_rcv_amp_gpio();
    } else {
...
static void msm8974_rcv_power_amp_off(u32 rcv)
{
    if (rcv & MSM8974_RCVAMP_ENABLE) {
...
static int msm8974_get_rcvamp(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm8974_rcv_control = %d", __func__, msm8974_rcv_control);
    ucontrol->value.integer.value[0] = msm8974_rcv_control;
	return 0;
}

static int msm8974_set_rcvamp(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	pr_debug("%s()\n", __func__);
	if (msm8974_rcv_control == ucontrol->value.integer.value[0])
		return 0;

	msm8974_rcv_control = ucontrol->value.integer.value[0];
	msm8974_ext_rcv_control(codec);
	return 1;
}


static int msm8974_rcvramp_event(struct snd_soc_dapm_widget *w,
			     struct snd_kcontrol *k, int event)
{
	pr_debug("%s()\n", __func__);
    ...
    if (SND_SOC_DAPM_EVENT_ON(event)) {
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
            msm8974_rcv_power_amp_on(MSM8974_RCVAMP_ENABLE);
        } else {
        ...
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
            msm8974_rcv_power_amp_off(MSM8974_RCVAMP_ENABLE);
        } else {
...
static const struct snd_soc_dapm_widget msm8974_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SPK("Ext Rcv Pos", msm8974_rcvramp_event),
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("Receiver Function", msm_snd_enum[6], msm8974_get_rcvamp,
		msm8974_set_rcvamp),
};
...
static __devinit int msm8974_asoc_machine_probe(struct platform_device *pdev)
{
	...
	piezo_rcvamp_gpio = of_get_named_gpio(pdev->dev.of_node,
				"qcom,rcv-amp-gpios", 0);
	if (piezo_rcvamp_gpio < 0) {
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed %d\n",
			"qcom, rcv-amp-gpios", pdev->dev.of_node->full_name,
			piezo_rcvamp_gpio);
		ret = -ENODEV;
		goto err;
	}
